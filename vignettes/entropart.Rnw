...
If it is an \code{AbdVector} (or an integer vector), the bias-corrected estimator is used (\emph{e.g.}\ \code{bcTsallis}).

The different ways to use the functions are a matter of personal preference.
\code{bcTsallis} with a numeric vector:

<<bcShannonP6.2>>=
bcTsallis(Paracou618.MC$Ns, q = 1)
@

is equivalent to \code{Tsallis} with an abundance vector:
<<bcShannonP6.3>>=
Tsallis(PAbd, q = 1)
@
...
Hurlbert's index is calculated by the \code{Hurlbert} function, its unbiased estimator by 
\code{bcHurlbert} and its effective number of species by \code{HurlbertD} or \code{bcHurlbertD}.
