...
The same relation holds between Ricotta and Szeidl entropy of a community \citep{Ricotta2006} and similarity-based diversity \citep{Leinster2012}.
...
It is an effective number of communities, \emph{i.e.}, the number of equally-weighted communities with no species in common necessary to obtain the same diversity as the data.
...
The best correction for Tsallis entropy follows \cite{Marcon2014}. 
\code{bcSimpson} returns Lande's correction \citep{Lande1996} and \code{bcShannon} returns the very efficient correction by \cite{Chao2013}, so their results are different (and more accurate) than those of the general \code{bcTsallis} function.
...
The effective number of species of the Paracou dataset is estimated to be \Sexpr{round(bcDiversity(Ns = Paracou618.MC$Ns, q = 2))} after bias correction (rather than \Sexpr{round(Diversity(Ps = Paracou618.MC$Ps, q = 2))} without it). It means that a community made of \Sexpr{round(bcDiversity(Ns = Paracou618.MC$Ns, q = 2))} equally-frequent species has the same Simpson entropy as the actual one. This is much less than the actual \Sexpr{round(Paracou618.MC$Nspecies, 0)} sampled species but Simpson's entropy focuses on dominant species. 
...
\begin{equation}
^q_i\!H_{\alpha}=\frac{1-\sum_s{p^q_{s,i}}}{q-1}=-\sum_s{p^q_{s,i}}\ln_q{p_{s,i}}
\label{Halpha}
\end{equation}
...
\begin{equation}
^q\!H_{\beta}=^q\!H_{\gamma}-^q\!H_{\alpha}=\sum_s{p^q_{s,i}\ln_q\frac{p_{s,i}}{p_s}}
\label{Hbeta}
\end{equation}
...
Unnormalized values are multiplied by the tree height, such as $^q\!\mathit{PD}(T)$ \citep{Chao2010}.
...
The latter is the diversity of the first slice of the tree. When going up the tree, diversity decreases because species collapse. On Figure~\ref{PhyloDiversity}, diversity of the second slice, between $T=1$ and $T=2$, is that of genera (\Sexpr{round(phd$Cuts[2], 0)} effective genera) and the last slice contains (\Sexpr{round(phd$Cuts[3], 0)} effective families). 
...
The \code{bcDqz} function allows estimation-bias correction.
...
Functional diversity of order 2 is only \Sexpr{round(bcDqz(Paracou618.MC$Ns, q = 2, Z), 2)} effective species, which is very small compared to \Sexpr{round(Diversity(Paracou618.MC$Ps, q = 2), 0)} effective species for Simpson diversity. 
\Sexpr{round(bcDqz(Paracou618.MC$Ns, q = 2, Z), 2)} equally-frequent species with similarity equal to 0 would have the same functional diversity as the actual community (made of \Sexpr{round(Paracou618.MC$Nspecies, 0)} species). 
...
The arguments of \code{EntropyCI} are an entropy function (any entropy function of the package accepting a vector of species abundances, such as \code{bcTsallis}), the number of simulations to run and the observed species frequencies. 
The result is a numeric vector containing the entropy value of each simulated community. 
Entropy can be finally transformed into diversity (but it is not correct to use a diversity function in simulations because the average simulated value must be calculate and only entropy can be averaged).
...
It returns a list which can be plotted. 
...
Several meta-communities, combined in a list, can be merged two different ways: the \code{MergeMC} function simplifies hierarchical partitioning of diversity: it creates a new meta-community whose communities are the original meta-communities aggregated data. 
...
The entropart package allows estimating biodiversity according to the framework based on HCDT entropy, the correction of its estimation-bias \citep{Grassberger1988, Chao2003} and its transformation into equivalent numbers of species \citep{Hill1973, Jost2006, Marcon2014}. 
...
This work has benefited from an "Investissement d'Avenir" grant managed by Agence Nationale de la Recherche (CEBA, ref. 
ANR-10-LABX-0025).
