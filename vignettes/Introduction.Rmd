...
If an ultrametric dendrogram describing species' phylogeny (here, a mere taxonomy with family, genus and species) is available, phylogenetic entropy and diversity  [@Marcon2014a] can be calculated:
...
```{r PDiversityProfile, cache = TRUE}
...
The `DivEst` function decomposes diversity and estimates confidence interval of $\alpha$, $\beta$ and $\gamma$ diversity following @Marcon2012. 
...
The bootstrap confidence intervals of the values of diversity [@Marcon2012; @Marcon2014] are calculated if `NumberOfSimulations` is not 0.
