...
function(Tree, q = 2, CheckArguments = TRUE) 
{
  ...
  Originality <- originality(phyTree)
  ...
  if (q != 2) {
    ...
    Optim <- optim(Freq, function(Ns, q, Tree, CheckArguments) bcPhyloEntropy(Ns, q, Tree, Correction="None", Normalize=FALSE)$Total, gr=NULL, q=q, Tree=Tree, lower=rep(0, length(Freq)), upper=rep(1, length(Freq)), method="L-BFGS-B", control=list(fnscale=-1))
    # Normalize
